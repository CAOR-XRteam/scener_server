...
class TestSession:
    ...
    async def test_run_success(self, mock_logger, mock_session):
        ...
        mock_logger.info.assert_called_once()
        assert (
            f"Session {mock_session.thread_id} cancelled"
            in mock_logger.info.call_args[0][0]
        )
    ...
    async def test_run_other_exception(self, mock_logger, mock_session):
        ...
        handle_message_mock = AsyncMock(side_effect=process_error)
        mock_session.handle_message = handle_message_mock
        ...
        mock_logger.error.assert_called_once_with(f"Session error: {process_error}")
    ...
    @pytest.mark.asyncio
    @patch("server.session.logger")
    async def test_handle_message_success(self, mock_logger, mock_agent, mock_session):
        message = InputMessage(command="chat", message="test test test")
        tokens = ["test ", "has...", "passed"]

        mock_session.client.send_message = AsyncMock()

        async def _mock_achat_gen(*args, **kwargs):
            for token in tokens:
                await asyncio.sleep(0)
                yield token

        mock_agent.achat.return_value = _mock_achat_gen()
        ...
        mock_agent.achat.assert_called_once_with(
...
class TestClient:
    ...
    async def test_send_message_other_exception(
        ...
        mock_client.queue_output.put = AsyncMock()
        mock_client.queue_output.put.side_effect = err
        message = OutputMessage(status="stream", code=200, message="test")
